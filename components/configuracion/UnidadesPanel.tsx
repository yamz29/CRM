              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select value={addForm.tipo} onChange={(e) => setAddForm((f) => ({ ...f, tipo: e.target.value }))}
                    className="w-full border border-border rounded-md px-3 h-8 text-sm bg-card focus:outline-none focus:ring-2 focus:ring-ring">
                    {TIPOS.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
                  </select>
                </div>
              </div>
              <div className="flex justify-end gap-2">
                <button onClick={() => setShowAddForm(false)} className="px-3 py-1.5 text-sm text-muted-foreground hover:text-foreground">
                  <X className="w-4 h-4" />
                </button>
            ...
            <div key={tipo}>
              ...
              <table className="w-full text-sm border border-border rounded-lg overflow-hidden">
                <thead className="bg-muted/40">
                  <tr className="text-xs text-muted-foreground font-semibold uppercase">
                        ...
                        <>
                          ...
                          <td className="px-2 py-1.5 text-center">
                            <select value={editForm.tipo} onChange={(e) => setEditForm((f) => ({ ...f, tipo: e.target.value }))}
                              className="w-full border border-border rounded px-1 h-7 text-xs bg-card">
                              {TIPOS.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
                            </select>
                          </td>
                          <td className="px-2 py-1.5">
                            <div className="flex gap-1 justify-end">
                              ...
                              <button onClick={() => setEditId(null)}
                                className="p-1 text-muted-foreground hover:bg-muted rounded"><X className="w-3.5 h-3.5" /></button>
                            </div>
                          </td>
                        </>
                      ) : (
                        <>
                          <td className="px-3 py-2 font-mono text-xs text-muted-foreground">{u.codigo}</td>
                          <td className="px-3 py-2 text-foreground">{u.nombre}</td>
                          <td className="px-3 py-2 text-center text-muted-foreground">{u.simbolo || '—'}</td>
                          <td className="px-3 py-2 text-center">
                            <button onClick={() => handleToggle(u)}
                              className={`text-xs font-medium px-2 py-0.5 rounded-full transition-colors ${u.activo ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-muted text-muted-foreground hover:bg-muted'}`}>
