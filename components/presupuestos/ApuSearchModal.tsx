        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-base font-bold text-slate-800">Catálogo de APUs</h2>
                <p className="text-xs text-slate-500">Selecciona un APU para insertarlo como partida</p>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {!loading && filtered.length === 0 && (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                <FileSpreadsheet className="w-10 h-10 text-slate-200 mb-3" />
                <p className="text-slate-400 text-sm">No se encontraron APUs</p>
                {query && <p className="text-slate-400 text-xs mt-1">Intenta con otra búsqueda</p>}
              </div>
            )}

            {!loading && Object.entries(grouped).map(([cap, items]) => (
              <div key={cap}>
                <div className="sticky top-0 px-6 py-1.5 bg-slate-50 border-b border-slate-100 text-xs font-semibold text-slate-500 uppercase tracking-wide">
                  {cap}
                </div>
                {items.map((apu) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-4 px-6 py-3 border-b border-slate-50 text-left transition-colors ${
                        isSelected
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          {apu.codigo && (
                            <span className="text-xs font-mono text-slate-400">{apu.codigo}</span>
                          )}
                          <span className="text-sm font-medium text-slate-800 truncate">{apu.nombre}</span>
                        </div>
                        {apu.descripcion && (
                          <p className="text-xs text-slate-400 mt-0.5 truncate">{apu.descripcion}</p>
                        )}
                      ...
                      <div className="text-right flex-shrink-0">
                        ...
                        <p className="text-xs text-slate-400">/{apu.unidad}</p>
                      </div>
                    </button>
                  )
                })}
          ...
          <div className="px-6 py-4 border-t border-slate-200 bg-slate-50">
            {selected ? (
              <div className="flex items-center gap-4">
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-slate-500 mb-0.5">APU seleccionado</p>
                  <p className="text-sm font-semibold text-slate-800 truncate">{selected.nombre}</p>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    min="0.001"
                    step="0.5"
                    className="w-20 border border-slate-300 rounded-lg px-2 py-1.5 text-sm text-right focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                  <span className="text-xs text-slate-500">{selected.unidad}</span>
                </div>
                <div className="text-right flex-shrink-0">
                  <p className="text-xs text-slate-500">Subtotal</p>
                  <p className="text-base font-bold text-slate-800">{formatCurrency(cantidad * selected.precioVenta)}</p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <p className="text-sm text-slate-400">Selecciona un APU de la lista para continuar</p>
