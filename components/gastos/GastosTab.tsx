...
const TIPO_COLORS: Record<string, string> = {
  ...
  'Gasto menor': 'bg-slate-100 text-slate-600',
...
function PartidaCell({ gasto, partidas, proyectoId, onUpdated }: {
  ...
  if (partidas.length === 0) {
    return <span className="text-xs text-slate-300 italic">Sin presupuesto</span>
  }
    ...
    <div ref={ref} className="relative">
      <button
        ...
        className={`text-xs px-2 py-1 rounded border transition-colors text-left w-full max-w-[180px] block truncate ${
          ...
          saving   ? 'border-slate-200 bg-slate-50 text-slate-400 cursor-wait' :
          label    ? 'border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-100' :
                     'border-dashed border-slate-300 text-slate-400 hover:border-blue-300 hover:text-blue-500'
        }`}
      >
        ...
        <div className="absolute top-full left-0 mt-1 w-72 bg-white border border-slate-200 rounded-lg shadow-xl z-50">
          ...
          <div className="max-h-56 overflow-y-auto py-1">
            {gasto.partida && (
              <button
                onClick={() => assign(null)}
                className="w-full text-left px-3 py-1.5 text-xs text-red-500 hover:bg-red-50 border-b border-slate-100"
              >
                ✕ Quitar asignación
              </button>
            )}
            {filteredPartidas.length === 0 ? (
              <div className="px-3 py-2.5 text-xs text-slate-400 text-center">Sin resultados</div>
            ) : (
              filteredPartidas.map(p => (
                <button
                  ...
                  className={`w-full text-left px-3 py-1.5 hover:bg-blue-50 transition-colors ${
                    gasto.partidaId === p.id ? 'bg-blue-50 text-blue-700' : 'text-slate-700'
                  }`}
                >
                  ...
                  {p.capituloNombre && (
                    <div className="text-xs text-slate-400 truncate">{p.capituloNombre}</div>
                  )}
                </button>
              ))
            )}
...
function ColumnPicker({ cols, onChange }: {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 w-52 bg-white border border-slate-200 rounded-lg shadow-xl z-40 py-1">
          <div className="px-3 py-1.5 text-xs font-semibold text-slate-500 border-b border-slate-100 mb-0.5">
            Columnas visibles
          </div>
          {Object.keys(COL_LABELS).map(key => {
            ...
            return (
              <label
                key={key}
                className={`flex items-center gap-2 px-3 py-1.5 hover:bg-slate-50 select-none ${always ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
              >
                <input
                  type="checkbox"
                ...
                />
                <span className="text-xs text-slate-700">{COL_LABELS[key]}</span>
                {always && <span className="text-xs text-slate-300 ml-auto">fija</span>}
              </label>
            )
          })}
        </div>
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          ...
          <p className="text-xl font-black text-slate-800">{formatCurrency(totalGastado)}</p>
          <p className="text-xs text-slate-400 mt-0.5">{gastosActivos.length} gastos activos</p>
        </div>
        ...
        {presupuestoEstimado != null && (
          <div className="bg-white border border-slate-200 rounded-xl p-4">
            ...
            <p className="text-xl font-black text-slate-800">{formatCurrency(presupuestoEstimado)}</p>
          </div>
        )}
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Total registros</span>
          </div>
          <p className="text-xl font-black text-slate-800">{gastos.length}</p>
          <p className="text-xs text-slate-400 mt-0.5">{gastos.filter(g => g.estado === 'Anulado').length} anulados</p>
        </div>
      </div>

      {/* ── Classification summary bar ── */}
      {gastosActivos.length > 0 && (
        <div className="bg-white border border-slate-200 rounded-xl px-4 py-2.5 flex items-center gap-5 flex-wrap">
          <div className="flex items-center gap-1.5">
            <Tag className="w-3.5 h-3.5 text-slate-400" />
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Clasificación</span>
          </div>
          <div className="flex items-center gap-1.5">
            <CheckCircle className="w-3.5 h-3.5 text-green-500" />
            <span className="text-xs text-slate-700"><span className="font-bold">{gastosConPartida}</span> asignados</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded-full border-2 border-dashed border-slate-300 inline-block flex-shrink-0" />
            <span className="text-xs text-slate-500"><span className="font-bold">{gastosSinPartida}</span> sin asignar</span>
          </div>
          <div className="flex items-center gap-2 ml-auto">
            <div className="h-1.5 w-28 bg-slate-100 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all ${pctClasificado === 100 ? 'bg-green-500' : pctClasificado > 0 ? 'bg-blue-500' : 'bg-slate-200'}`}
                style={{ width: `${pctClasificado}%` }}
              />
            </div>
            <span className="text-xs font-bold text-slate-700">{pctClasificado}%</span>
          </div>
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          {filtroActivo && (
            <button onClick={clearFilters} className="text-xs text-slate-400 hover:text-slate-600">Limpiar</button>
          )}
        </div>
      ...
      {showFilters && (
        <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div>
              <label className="text-xs font-medium text-slate-600 block mb-1">Tipo de gasto</label>
            ...
            {partidas.length > 0 && (
              <div className="md:col-span-2">
                <label className="text-xs font-medium text-slate-600 block mb-1">Partida presupuestaria</label>
        ...
        <div className="flex items-center gap-3 bg-blue-50 border border-blue-200 rounded-xl px-4 py-2.5 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-1 min-w-0">
            <select
              ...
              className="h-8 text-xs border border-blue-200 rounded-md px-2 bg-white flex-1 max-w-xs"
            >
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        {loading ? (
          <div className="py-16 text-center text-sm text-slate-400">Cargando gastos...</div>
        ) : filtered.length === 0 ? (
          ...
          <>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    ...
                    {col('fecha')      && <th className="px-3 py-2.5 text-left text-xs font-semibold text-slate-500 uppercase whitespace-nowrap">Fecha</th>}
                    {col('tipo')       && <th className="px-3 py-2.5 text-left text-xs font-semibold text-slate-500 uppercase">Tipo</th>}
                    {col('referencia') && <th className="px-3 py-2.5 text-left text-xs font-semibold text-slate-500 uppercase">Ref.</th>}
                    <th className="px-3 py-2.5 text-left text-xs font-semibold text-slate-500 uppercase">Descripción</th>
                    {col('suplidor')   && <th className="px-3 py-2.5 text-left text-xs font-semibold text-slate-500 uppercase">Suplidor</th>}
                    {col('categoria')  && <th className="px-3 py-2.5 text-left text-xs font-semibold text-slate-500 uppercase">Categoría</th>}
                    {col('pago')       && <th className="px-3 py-2.5 text-left text-xs font-semibold text-slate-500 uppercase">Pago</th>}
                    {col('partida')    && <th className="px-3 py-2.5 text-left text-xs font-semibold text-slate-500 uppercase">Partida</th>}
                    <th className="px-3 py-2.5 text-right text-xs font-semibold text-slate-500 uppercase">Monto</th>
                    {col('estado')     && <th className="px-3 py-2.5 text-center text-xs font-semibold text-slate-500 uppercase">Estado</th>}
                    {col('adjunto')    && <th className="px-3 py-2.5 text-center text-xs font-semibold text-slate-500 uppercase">Doc.</th>}
                    <th className="px-3 py-2.5 text-right text-xs font-semibold text-slate-500 uppercase">Acc.</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {filtered.map(g => (
                    <tr key={g.id} className={`hover:bg-slate-50/60 ${g.estado === 'Anulado' ? 'opacity-50' : ''} ${selectedIds.has(g.id) ? 'bg-blue-50/60' : ''}`}>
                      ...
                      {col('fecha') && (
                        <td className="px-3 py-2 text-xs text-slate-600 whitespace-nowrap">{formatDate(g.fecha)}</td>
                      )}
                      {col('tipo') && (
                        <td className="px-3 py-2">
                          <span className={`inline-flex px-1.5 py-0.5 rounded-full text-xs font-semibold ${TIPO_COLORS[g.tipoGasto] ?? 'bg-slate-100 text-slate-600'}`}>
                            {g.tipoGasto}
                          </span>
                        </td>
                      )}
                      {col('referencia') && (
                        <td className="px-3 py-2 text-xs text-slate-500 font-mono">{g.referencia || '—'}</td>
                      )}
                      <td className="px-3 py-2 text-sm text-slate-700 max-w-[180px]">
                        <span className="truncate block" title={g.descripcion}>{g.descripcion}</span>
                        {g.observaciones && (
                          <span className="text-xs text-slate-400 truncate block">{g.observaciones}</span>
                        )}
                      </td>
                      {col('suplidor') && (
                        <td className="px-3 py-2 text-xs text-slate-600 max-w-[120px]">
                      ...
                      {col('categoria') && (
                        <td className="px-3 py-2 text-xs text-slate-500">
                          {g.categoria || '—'}
                          {g.subcategoria && <span className="block text-slate-400">{g.subcategoria}</span>}
                        </td>
                      )}
                      {col('pago') && (
                        <td className="px-3 py-2 text-xs text-slate-500 whitespace-nowrap">{g.metodoPago}</td>
                      )}
                      {col('partida') && (
                        <td className="px-3 py-2">
                          <PartidaCell
                      ...
                      <td className="px-3 py-2 text-right text-sm font-bold text-slate-800 tabular-nums whitespace-nowrap">
                        {g.moneda !== 'RD$' && <span className="text-xs text-slate-400 mr-1">{g.moneda}</span>}
                        {formatCurrency(g.monto)}
                      </td>
                      {col('estado') && (
                        <td className="px-3 py-2 text-center">
                          <span className={`inline-flex px-1.5 py-0.5 rounded-full text-xs font-semibold ${ESTADO_COLORS[g.estado] ?? 'bg-slate-100 text-slate-600'}`}>
                            {g.estado}
                          </span>
                        </td>
                      )}
                        ...
                        <div className="flex justify-end gap-0.5">
                          ...
                          {g.estado !== 'Anulado' && (
                            <button onClick={() => handleAnular(g.id)}
                              className="p-1.5 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded transition-colors" title="Anular">
            ...
            <div className="px-4 py-3 bg-slate-50 border-t border-slate-200 flex items-center justify-between">
              ...
              <div className="text-right">
                <span className="text-xs text-slate-500 mr-2">Total filtrado (activos):</span>
